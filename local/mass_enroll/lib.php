...
function mass_enroll($cir, $course, $context, $data) {
  ...
  $userid = $USER->id;
  $sql = "SELECT c.id, c.fullname from {course} as c left join "
      . "{course_categories} as cc on c.category = cc.id left join "
      . "{company} as com on com.category = c.category left join "
      . "{company_users} as comu on com.id = comu.companyid where comu.userid = $userid";
  $records = $DB->get_records_sql($sql);
  $courseinstance = array();
  foreach ($records as $key => $value) {
    $course = $DB->get_record('course', array('id' => $value->id));
    $coursename = strtolower(str_replace('"', '', trim($value->fullname)));
    $courseinstance[$coursename] = $course;
    ...
    $instancearray[$coursename] = $instance;
  }
  ...
  while ($fields = $cir->next()) {
    ...
    $course_name = strtolower(str_replace('"', '', trim($fields[0])));
    $course = $courseinstance[$course_name];
    $instance = $instancearray[$course_name];
    if (!$user = $DB->get_record('user', array($useridfield => $fields[1]))) {
      $result .= get_string('im:user_unknown', 'local_mass_enroll', $fields[1]) . "\n";
      continue;
    }
    // Already enroled?
