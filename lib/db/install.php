...
function xmldb_main_install() {
    ...
    $clientadministratorid = create_role('Client Administrator', 'clientadministrator',
                                         '() Manages site - can create new companies and add managers etc.', 'clientadministrator');
    $companymanagerid = create_role('Company Manager', 'companymanager',
                                    '() Manages individual companies - can upload users etc.', 'companymanager');
    $companydepartmentmanagerid = create_role('Company Department Manager', 'companydepartmentmanager',
                                              ' Manages departments within companies - can upload users etc.', 'companydepartmentmanager');
    $companycourseeditorid = create_role('Client Course Editor', 'companycourseeditor',
                                         ' Client Course Editor - can edit course content; add, delete, modify etc..', 'companycourseeditor');
    $companycoursenoneditorid = create_role('Client Course Access', 'companycoursenoneditor',
                                            ' Client Course Access - similar to the non-editing teacher role for client admin', 'companycoursenoneditor');
    $clientreporterid = create_role('Client Reporter Only', 'clientreporter',
                                            ' Client Report Only  - Similar to the client admin roles but allows for access to the reports only', 'clientreporter');
    $companyreporterid = create_role('Company Reporter Only', 'companyreporter',
                                            ' Company Report Only - similar to the company manager role but allows for access to the company reports only', 'companyreporter');
